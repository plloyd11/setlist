-- ============================================================================
-- Setlist gaps: a setlist_songs row is either a song reference or a timed
-- break ("gap") between songs. Gap rows have song_id null + gap_seconds set;
-- song rows keep gap_seconds null. Backward compatible: every existing row is
-- a song row, and clients that never send gap_seconds keep working.
-- ============================================================================

alter table public.setlist_songs alter column song_id drop not null;

alter table public.setlist_songs add column gap_seconds integer;

alter table public.setlist_songs
  add constraint setlist_songs_song_or_gap check (
    (song_id is not null and gap_seconds is null)
    or (song_id is null and gap_seconds > 0)
  );

-- ============================================================================
-- save_setlist_order: accept gap items. p_items is an ordered jsonb array of
-- {id, song_id, gap_seconds}; id null/absent means a new row. Song visibility
-- is only checked for new *song* entries — gap entries reference no song.
-- ============================================================================
...
begin
  ...
  insert into public.setlist_songs (setlist_id, song_id, gap_seconds, position)
  select
    p_setlist_id,
    (e.elem->>'song_id')::uuid,
...
-- ============================================================================
-- get_shared_setlist: include song notes (stage cues like count-ins belong on
-- the shared/printed sheet) and gap rows. Gap entries carry gap_seconds only;
-- song entries carry title + notes.
-- ============================================================================
...
begin
  ...
  select coalesce(
    jsonb_agg(
      case
        when ss.song_id is null then jsonb_build_object('gap_seconds', ss.gap_seconds)
